...
const schemas = {
  payment: Joi.object({
    ...
    phoneNumber: Joi.when('paymentGateway', {
      ...
      otherwise: Joi.string().pattern(/^\+?[0-9]{10,15}$/).optional()
    }),
    description: Joi.string().max(255).optional(),
    tripId: Joi.string().uuid().required(),
    paymentGateway: Joi.string().valid(...Object.values(PaymentGateway)).required(),
    paymentMethod: Joi.when('paymentGateway', {
      is: 'SIMPLE',
      then: Joi.string().valid(...Object.values(PaymentMethod)).required(),
  ...
  statusCheck: Joi.object({
    reference: Joi.string().pattern(/^[A-Z0-9-]+$/).required(),
